                ...
                <div class="shadow-box shadow-box-with-fixed-bottom-bar">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <!-- For Json Query / SNMP -->
                            <div v-if="monitor.type === 'snmp'" class="my-3">
                                <div class="p-3 bg-secondary-subtle border border-secondary rounded-3 mb-3">
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        <span class="fw-bold small text-uppercase">SNMP Telemetry Diagnostics</span>
                                        <button class="btn btn-warning btn-sm fw-bold d-flex align-items-center gap-2" type="button" :disabled="snmpTesting" @click="testSnmp">
                                            <font-awesome-icon icon="bolt" />
                                            <span>{{ snmpTesting ? "Testing SNMP..." : "Test SNMP Connection" }}</span>
                                        </button>
                                    </div>
                                    <div class="form-text small">Test real-time OID reachability and latency against target SNMP agent.</div>
                                </div>
                            </div>
                ...
                `
{
    ...
    methods: {
        ...
        testSnmp() {
            ...
            this.$root.getSocket().emit("testSnmpConnection", {
                hostname: this.monitor.hostname,
                port: this.monitor.port || 161,
                snmpVersion: this.monitor.snmpVersion || "v2c",
                community: this.monitor.snmpCommunity || "public",
                oid: this.monitor.jsonPath || "1.3.6.1.2.1.1.1.0",
            }, (res) => {
                this.snmpTesting = false;
                if (res && res.ok) {
                    this.$root.toastSuccess(res.msg || "SNMP connection successful!");
                } else {
                    this.$root.toastError((res && res.msg) || "SNMP connection test failed.");
                }
            });
        },
