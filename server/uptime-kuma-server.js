...
class UptimeKumaServer {
    ...
    /**
     * Dynamically read Index HTML from disk to ensure fresh asset hashes after builds
    ...
    get indexHTML() {
        try {
            ...
            return this._indexHTMLCache || "";
        }
    }

    /**
     * Set cached Index HTML value
     * @param {string} value Index HTML string
     */
    set indexHTML(value) {
        this._indexHTMLCache = value;
    }
    ...
    async initAfterDatabaseReady() {
        try {
            ...
            log.error("server", "Failed to load dist/index.html: " + e.message);
        }
    ...
    async sendMonitorList(socket) {
        const activeOrgId = socket ? (socket.activeOrganizationId || 1) : 1;
        let list = await this.getMonitorJSONList(socket.userID, null, activeOrgId);
    ...
    async sendUpdateMonitorIntoList(socket, monitorID) {
        const activeOrgId = socket ? (socket.activeOrganizationId || 1) : 1;
        let list = await this.getMonitorJSONList(socket.userID, monitorID, activeOrgId);
    ...
    /**
     * Get a list of monitors for the given user and active organization.
     * @param {string} userID - The ID of the user to get monitors for.
     * @param {number} monitorID - The ID of monitor for.
     * @param {number} activeOrgId - The active organization ID.
     * @returns {Promise<object>} A promise that resolves to an object with monitor IDs as keys and monitor objects as values.
     */
    async getMonitorJSONList(userID, monitorID = null, activeOrgId = 1) {
        let query = " (organization_id = ? OR (user_id = ? AND organization_id IS NULL)) ";
        let queryParams = [activeOrgId, userID];
     ...
     */
    getUserAgent() {
        return "InfiniNOC/" + require("../package.json").version;
    }
