...
async function sendNotificationList(socket) {
    ...
    let list = await R.find("notification", " organization_id = ? OR (user_id = ? AND organization_id IS NULL) ", [activeOrgId, socket.userID]);
...
async function sendProxyList(socket) {
    ...
    const activeOrgId = socket.activeOrganizationId || 1;
    const list = await R.find("proxy", " organization_id = ? OR (user_id = ? AND organization_id IS NULL) ", [activeOrgId, socket.userID]);
...
async function sendAPIKeyList(socket) {
    ...
    let result = [];
    const activeOrgId = socket.activeOrganizationId || 1;
    const list = await R.find("api_key", " organization_id = ? OR (user_id = ? AND organization_id IS NULL) ", [activeOrgId, socket.userID]);
...
async function sendDockerHostList(socket) {
    ...
    let result = [];
    const activeOrgId = socket.activeOrganizationId || 1;
    let list = await R.find("docker_host", " organization_id = ? OR (user_id = ? AND organization_id IS NULL) ", [activeOrgId, socket.userID]);
